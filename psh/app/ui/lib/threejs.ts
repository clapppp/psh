      ...
      function (gltf) {
        ...
        function animate(time: DOMHighResTimeStamp) {
          ...
          if (deltaTime > interval) {
            ...
            gltf.scene.position.x = Math.min(
              Math.max(gltf.scene.position.x, -15),
              15
            );
            gltf.scene.position.y = Math.min(
              Math.max(gltf.scene.position.y, -15),
              15
            );
            ...
            for (const name of gltfList.keys()) {
              ...
              if (nowPos.equals(prevPos) && gltf) {
                const pos = new Three.Vector3().addVectors(
                  gltf.scene.position,
                  cordList.get(name) ?? new Three.Vector3()
                );
                setPosition(gltf, pos, name);
              } else {
                if (gltf && user) setPosition(gltf, nowPos, name);
                if (!speedList.has(name))
                  speedList.set(name, new Three.Vector3(0, 0, 0));
                const nowSpeed = new Three.Vector3()
                  .subVectors(nowPos, prevPos)
                  .divideScalar(0.1);
                accelList.set(
                  name,
                  new Three.Vector3()
                    .subVectors(
                      nowSpeed,
                      speedList.get(name) ?? new Three.Vector3()
                    )
                    .divideScalar(cycle / 1000)
                );
                speedList.set(name, nowSpeed);
                const nextPos = new Three.Vector3().addVectors(
                  new Three.Vector3()
                    .addVectors(
                      nowSpeed,
                      accelList.get(name) ?? new Three.Vector3()
                    )
                    .multiplyScalar(cycle / 1000),
                  nowPos
                );
                cordList.set(
                  name,
                  new Three.Vector3()
                    .subVectors(nextPos, nowPos)
                    .divideScalar(divideFrame)
                );
